...
public class XXH3SanityTest {
  ...
  @Test
  @DisplayName("XXH3 64-bit: nonzero seed covers every 1-3 byte length")
  void testXXH3_64bits_seeded_1to3() {
    for (int len = 1; len <= 3; len++) {
      final int expectedLen = len;
      assertTrue(xxh3_64BitsVectors.stream().anyMatch(
        v -> v.len == expectedLen && v.seed != 0));
    }
  }
  ...
  @Test
  @DisplayName("XXH3 128-bit: nonzero seed covers every 1-3 byte length")
  void testXXH3_128bits_seeded_1to3() {
    for (int len = 1; len <= 3; len++) {
      final int expectedLen = len;
      assertTrue(xxh3_128BitsVectors.stream().anyMatch(
        v -> v.len == expectedLen && v.seed != 0));
    }
  }
  ...
  @Test
  @DisplayName("caller-owned 128-bit output matches allocating APIs")
  void testHash128FoldedInto() {
    int[] lengths = {0, 1, 3, 4, 8, 9, 16, 17, 128, 129, 240, 241, 4096};
    long[] seeds = {0L, PRIME64};
    for (long seed : seeds) {
      for (int len : lengths) {
        byte[] input = inputOf(len);
        byte[] expected = XXH3.hashUnsafeBytes128(
            input, Platform.BYTE_ARRAY_OFFSET, len, seed);
        byte[] output = new byte[16];
        long folded = XXH3.hashUnsafeBytes128FoldedInto(
            input, Platform.BYTE_ARRAY_OFFSET, len, seed, output);
        assertArrayEquals(expected, output, "caller-owned output mismatch for len=" + len);
        assertEquals(XXH3.fold128(output), folded, "fold mismatch for len=" + len);

        Arrays.fill(output, (byte) 0x5A);
        folded = XXH3.hashBytes128FoldedInto(input, 0, len, seed, output);
        assertArrayEquals(expected, output, "byte-array slice output mismatch for len=" + len);
        assertEquals(XXH3.fold128(output), folded, "byte-array slice fold mismatch for len=" + len);
      }
    }

    byte[] output = new byte[16];
    long intFold = XXH3.hashInt128FoldedInto(1234, PRIME64, output);
    assertArrayEquals(XXH3.hashInt128(1234, PRIME64), output);
    assertEquals(XXH3.fold128(output), intFold);
    long longFold = XXH3.hashLong128FoldedInto(123456789L, PRIME64, output);
    assertArrayEquals(XXH3.hashLong128(123456789L, PRIME64), output);
    assertEquals(XXH3.fold128(output), longFold);
  }
  ...
  @Test
  @DisplayName("Safe byte-array APIs reject invalid slices and output buffers")
  void testSafeApiBoundsChecks() {
    byte[] input = {1, 2, 3};
    assertThrows(IndexOutOfBoundsException.class, () -> XXH3.hashBytes64(input, 1, 3));
    assertThrows(IndexOutOfBoundsException.class, () -> XXH3.hashBytes64(input, 0, -1));
    assertThrows(IndexOutOfBoundsException.class, () -> XXH3.hashBytes128(input, -1, 1));
    assertThrows(IndexOutOfBoundsException.class,
      () -> XXH3.hashBytes128FoldedInto(input, 0, input.length, 0L, new byte[15]));
    assertThrows(IndexOutOfBoundsException.class,
      () -> XXH3.hashUnsafeBytes128FoldedInto(
        input, Platform.BYTE_ARRAY_OFFSET, input.length, 0L, new byte[15]));
    assertThrows(IndexOutOfBoundsException.class,
      () -> XXH3.hashInt128FoldedInto(1, 0L, new byte[15]));
    assertThrows(IndexOutOfBoundsException.class,
      () -> XXH3.hashLong128FoldedInto(1L, 0L, new byte[15]));
  }

  @Test
  @DisplayName("Safe slices and generic unsafe bases match exactly-sized byte arrays")
  void testSliceOffsetsAndUnsafeBases() {
    byte[] expected = inputOf(257);
    byte[] padded = new byte[expected.length + 11];
    System.arraycopy(expected, 0, padded, 5, expected.length);

    assertEquals(XXH3.hashBytes64(expected, 0, expected.length, PRIME32),
      XXH3.hashBytes64(padded, 5, expected.length, PRIME32));
    assertArrayEquals(XXH3.hashBytes128(expected, 0, expected.length, PRIME32),
      XXH3.hashBytes128(padded, 5, expected.length, PRIME32));

    int[] genericBase = new int[(expected.length + 3) / 4];
    Platform.copyMemory(expected, Platform.BYTE_ARRAY_OFFSET, genericBase,
      Platform.INT_ARRAY_OFFSET, expected.length);
    assertEquals(XXH3.hashUnsafeBytes64(
      expected, Platform.BYTE_ARRAY_OFFSET, expected.length, PRIME32),
      XXH3.hashUnsafeBytes64(
        genericBase, Platform.INT_ARRAY_OFFSET, expected.length, PRIME32));
    assertArrayEquals(XXH3.hashUnsafeBytes128(
      expected, Platform.BYTE_ARRAY_OFFSET, expected.length, PRIME32),
      XXH3.hashUnsafeBytes128(
        genericBase, Platform.INT_ARRAY_OFFSET, expected.length, PRIME32));
  }

  @Test
  @DisplayName("Unsafe hash APIs support true off-heap input")
  void testOffHeapInput() {
    byte[] input = inputOf(257);
    long address = Platform.allocateMemory(input.length);
    try {
      Platform.copyMemory(input, Platform.BYTE_ARRAY_OFFSET, null, address, input.length);
      assertEquals(XXH3.hashUnsafeBytes64(
        input, Platform.BYTE_ARRAY_OFFSET, input.length, PRIME32),
        XXH3.hashUnsafeBytes64(null, address, input.length, PRIME32));
      assertArrayEquals(XXH3.hashUnsafeBytes128(
        input, Platform.BYTE_ARRAY_OFFSET, input.length, PRIME32),
        XXH3.hashUnsafeBytes128(null, address, input.length, PRIME32));
    } finally {
