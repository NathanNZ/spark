...
class HashExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("xxh3 interpreted and codegen parity for physical UDT types") {
    class StringUDT extends UserDefinedType[String] {
      override def sqlType: DataType = StringType
      override def serialize(obj: String): Any = UTF8String.fromString(obj)
      override def deserialize(datum: Any): String = datum.asInstanceOf[UTF8String].toString
      override def userClass: Class[String] = classOf[String]
    }
    class DecimalUDT extends UserDefinedType[java.math.BigDecimal] {
      override def sqlType: DataType = DecimalType(20, 4)
      override def serialize(obj: java.math.BigDecimal): Any = Decimal(obj, 20, 4)
      override def deserialize(datum: Any): java.math.BigDecimal =
        datum.asInstanceOf[Decimal].toJavaBigDecimal
      override def userClass: Class[java.math.BigDecimal] = classOf[java.math.BigDecimal]
    }

    val stringUdt = new StringUDT
    val decimalUdt = new DecimalUDT
    val literals = Seq(
      Literal.create(stringUdt.serialize("hello"), stringUdt),
      Literal.create(decimalUdt.serialize(new java.math.BigDecimal("123456789.1234")), decimalUdt))
    literals.foreach { literal =>
      val hash64 = XxHash3(Seq(literal), 0L)
      checkEvaluation(hash64, hash64.eval(null))
      val hash128 = XxHash128(Seq(literal), 0L)
      checkEvaluation(hash128, hash128.eval(null))
    }
  }

  test("xxh3 geometry and geography codegen parity") {
    val bytes = Array[Byte](1, 2, 3, 4, 5)
    Seq[DataType](GeometryType(4326), GeographyType(4326)).foreach { dataType =>
      val literal = Literal.create(BinaryView.fromBytes(bytes), dataType)
      val hash64 = XxHash3(Seq(literal), 0L)
      checkEvaluation(hash64, hash64.eval(null))
      val hash128 = XxHash128(Seq(literal), 0L)
      checkEvaluation(hash128, hash128.eval(null))
  ...
  test("xxh3 geometry and geography support true off-heap BinaryView") {
    val bytes = Array[Byte](1, 2, 3, 4, 5)
    ...
    try {
      ...
      Seq[DataType](GeometryType(4326), GeographyType(4326)).foreach { dataType =>
        val onHeap = Literal.create(BinaryView.fromBytes(bytes), dataType)
        val offHeap = Literal.create(BinaryView.fromAddress(null, address, bytes.length), dataType)
        assert(XxHash3(Seq(onHeap), 0L).eval() == XxHash3(Seq(offHeap), 0L).eval())
        assert(java.util.Arrays.equals(
          XxHash128(Seq(onHeap), 0L).eval().asInstanceOf[Array[Byte]],
          XxHash128(Seq(offHeap), 0L).eval().asInstanceOf[Array[Byte]]))
        checkEvaluation(XxHash3(Seq(offHeap), 0L), XxHash3(Seq(onHeap), 0L).eval())
        checkEvaluation(XxHash128(Seq(offHeap), 0L), XxHash128(Seq(onHeap), 0L).eval())
      }
    } finally {
  ...
  // ---------------------------------------------------------------------------
  // XXH3-family contract: position-aware null runs, single-value interop, tombstone,
  ...
  test("xxhash3 trailing nulls perturb the result") {
    val x = Literal.create(1, IntegerType)
    val y = Literal.create(2, IntegerType)
    val z = Literal.create(null, IntegerType)
    assert(xxh3Eval(Seq(x, y)) != xxh3Eval(Seq(x, y, z)))
    assert(xxh3Eval(Seq(x, y, z)) != xxh3Eval(Seq(x, y, z, z)))
  }

  test("xxhash128 trailing nulls perturb the result") {
    val x = Literal.create(1, IntegerType)
    val y = Literal.create(2, IntegerType)
    val z = Literal.create(null, IntegerType)
    assert(!java.util.Arrays.equals(xxh128Eval(Seq(x, y)), xxh128Eval(Seq(x, y, z))))
    assert(!java.util.Arrays.equals(xxh128Eval(Seq(x, y, z)), xxh128Eval(Seq(x, y, z, z))))
  }
  ...
  test("xxhash3 single null returns tombstone and wider all-null inputs are structural") {
    val z = Literal.create(null, IntegerType)
    assert(xxh3Eval(Seq(z))       == 0L)
    assert(xxh3Eval(Seq(z, z))    != 0L)
    assert(xxh3Eval(Seq(z, z))    != xxh3Eval(Seq(z, z, z)))
  }

  test("xxhash128 single null returns tombstone and wider all-null inputs are structural") {
    val z = Literal.create(null, IntegerType)
    assert(java.util.Arrays.equals(xxh128Eval(Seq(z)), new Array[Byte](16)))
    assert(!java.util.Arrays.equals(xxh128Eval(Seq(z, z)), new Array[Byte](16)))
    assert(!java.util.Arrays.equals(xxh128Eval(Seq(z, z)), xxh128Eval(Seq(z, z, z))))
  }
  ...
  test("xxhash128 single-null tombstone is independent of internal seed") {
    val z = Literal.create(null, IntegerType)
    val expected = new Array[Byte](16)
    assert(java.util.Arrays.equals(
      XxHash128(Seq(z), 7L).eval(InternalRow.empty).asInstanceOf[Array[Byte]], expected))
  }
  ...
  test("xxhash3 and xxhash128 MapType codegen matches interpreted with null values") {
    ...
    val literal = Literal.create(map, mapType)
    val hash64 = new XxHash3(Seq(literal))
    val hash128 = new XxHash128(Seq(literal))
    checkEvaluation(hash64, hash64.eval(InternalRow.empty))
    checkEvaluation(hash128, hash128.eval(InternalRow.empty))
  }
  ...
  test("xxhash128 MapType: duplicate-key entries hash distinctly (addition, not XOR)") {
    ...
    def dupMap(k: String, v: Long) = Literal.create(
      ...
      mt)
    assert(!java.util.Arrays.equals(
      xxh128Eval(Seq(dupMap("a", 1L))), xxh128Eval(Seq(dupMap("b", 2L)))),
      "{a:1,a:1} and {b:2,b:2} must not collide - XOR would make both reduce to 0")
  }
  ...
  test("xxhash3 and xxhash128 codegen handle wide nested structs") {
    val nestedFields = Array.tabulate(40)(i => StructField(s"n$i", IntegerType, nullable = false))
    val nestedType = StructType(nestedFields)
    val dataType = StructType(Array(
      StructField("left", nestedType, nullable = false),
      StructField("right", nestedType, nullable = false)))
    val left = new GenericInternalRow(Array.tabulate[Any](nestedFields.length)(identity))
    val right = new GenericInternalRow(
      Array.tabulate[Any](nestedFields.length)(i => i + nestedFields.length))
    val value = new GenericInternalRow(Array[Any](left, right))
  ...
  test("Support TimeType") {
    ...
    checkEvaluation(XxHash128(Seq(time), 10L), XxHash128(Seq(time), 10L).eval())
  ...
  test("HashExpression supports nanosecond timestamp types") {
    ...
    Seq(TimestampNTZNanosType(9), TimestampLTZNanosType(9),
      ...
      (values :+ null).foreach { v =>
        ...
        checkEvaluation(XxHash3(Seq(lit), 42L), XxHash3(Seq(lit), 42L).eval())
        checkEvaluation(XxHash128(Seq(lit), 42L), XxHash128(Seq(lit), 42L).eval())
        ...
        checkEvaluation(XxHash3(Seq(ref), 42L), XxHash3(Seq(ref), 42L).eval(row), row)
        checkEvaluation(XxHash128(Seq(ref), 42L), XxHash128(Seq(ref), 42L).eval(row), row)
  ...
  test("SPARK-58236: CalendarInterval hash folds all fields and agrees across codegen") {
    ...
    def assertXxHash3Fields(hash: Literal => Long): Unit = {
      assert(hash(base) === hash(lit(1, 5, 7L)))
      assert(hash(base) !== hash(lit(2, 5, 7L))) // months
      assert(hash(base) !== hash(lit(1, 6, 7L))) // days
      assert(hash(base) !== hash(lit(1, 5, 8L))) // microseconds
    }
    assertXxHash3Fields(e => XxHash3(Seq(e), 42L).eval().asInstanceOf[Long])

    def assertXxHash128Fields(hash: Literal => Array[Byte]): Unit = {
      assert(java.util.Arrays.equals(hash(base), hash(lit(1, 5, 7L))))
      assert(!java.util.Arrays.equals(hash(base), hash(lit(2, 5, 7L)))) // months
      assert(!java.util.Arrays.equals(hash(base), hash(lit(1, 6, 7L)))) // days
      assert(!java.util.Arrays.equals(hash(base), hash(lit(1, 5, 8L)))) // microseconds
    }
    assertXxHash128Fields(e => XxHash128(Seq(e), 42L).eval().asInstanceOf[Array[Byte]])
    ...
    Seq(lit(0, 5, 0L), lit(0, -5, 0L), lit(3, 5, 0L), lit(1, 5, 7L)).foreach { interval =>
      ...
      checkEvaluation(XxHash3(Seq(interval), 42L), XxHash3(Seq(interval), 42L).eval())
      checkEvaluation(XxHash128(Seq(interval), 42L), XxHash128(Seq(interval), 42L).eval())
